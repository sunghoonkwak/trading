...
이 파일은 애플리케이션의 메인 엔트리 포인트입니다. 인증, 로그 로테이션, 웹소켓 통신 및 터미널의 대화형 메뉴를 조율합니다.
...
#### output
- 백업된 로그 파일 (예: `WebSocket_20251228_120000.log`) 및 새로 생성된 `WebSocket_latest.log`.
...
### on_result
웹소켓 메시지에 대한 전역 콜백 함수입니다. 수신된 체결/시세 데이터를 파싱하여 전역 상태(`.trading_state`)를 업데이트하고 UI 로그를 트리거합니다.
#### input
- `ws`: 웹소켓 연결 객체.
