...
3. **Static File Serving**: `favicon.ico`, `index.html`, `styles.css`, `app.js` 등 프론트엔드 정적 파일을 제공합니다.
...
6. **Holdings Data API**: 특정 종목의 상세 보유 현황을 조회하기 위한 과 REST API(`/api/holdings/{ticker}`)를 제공합니다.
    - 데이터 접근 및 최신화(Freshness Check) 로직은 `data_service`를 통해 통합 관리됩니다.
...
### `favicon()`
브라우저의 기본 파비콘 요청(`/favicon.ico`)에 응답합니다.
- `src/web/favicon.ico` 파일을 읽어 반환합니다.
