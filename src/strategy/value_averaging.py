...
def calculate_orders(
    ...
    for ticker, config in targets_config.items():
        ...
        for entry in history_data:
            if ticker in entry.get('targets', {}).get('va', {}).get('targets_context', {}):
                # New unified format
                ticker_hist_entry = entry['targets']['va']['targets_context'][ticker]
                ticker_hist_date = entry['date']
                break
            elif ticker in entry.get('targets', {}):
        ...
        if using_existing_today:
            ...
            for res in results_list:
                # Use 'success' field, fallback to 'executed' for legacy data
                is_success = res.get('success', res.get('executed', False))
                if is_success and res.get('type') not in ['skip', None]:
                    executed_today = True
                    break
