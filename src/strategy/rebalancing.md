...
5. **중복 주문 방지**: 스케줄러가 5분마다 실행되지만, LOC 주문은 장 마감 시 체결되어 장중에는 미체결 상태로 남습니다. `rebalancing_history.json`에 당일 실행 이력이 있으면 스킵하여 동일 주문이 반복 실행되는 것을 방지합니다. (`execute=True` 시에만 작동, 텔레그램 `/rebalance` 조회에는 영향 없음)
