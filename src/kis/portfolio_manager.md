# KIS Portfolio Manager (`src/kis/portfolio_manager.py`)

KIS API에서 한투 계좌 자산을 조회하고 표준 source 포맷으로 변환하는
어댑터입니다. 전체 자산 통합과 GSheet 병합은
`data.portfolio_integration`이 담당합니다.
...
기존 호출자를 위한 호환 shim입니다. 실제 통합 로직은
`data.portfolio_integration.get_integrated_portfolio`로 위임합니다.
...
kis_raw = PortfolioManager._fetch_kis_account_data()
kis_source = PortfolioManager._convert_kis_to_standard(kis_raw)
```
