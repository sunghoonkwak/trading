# Portfolio Report Scheduler (`src/scheduler/scheduler_portfolio.py`)
...
- **Timeout Alerts**: 데이터 호출 시 지연 응답으로 인한 스케줄러 기능 마비에 대비하여, 포트폴리오 데이터를 불러오던 중 타임아웃 발생 시 즉각적으로 텔레그램으로 장애 알림을 노출합니다.
