...
1. **데일리 전략 (RAOEO/VA)**: 매일 밤 정해진 시간(예: 21:00)에
   ...
   조회를 반복하지 않습니다.
2. **주기적 리밸런싱**: US/Eastern 장중 시간(09:40 ~ 15:40 ET) 동안 5분 간격으로 비중을 체크하고 리밸런싱을 수행합니다. 리밸런싱 전략이 `disabled` 상태이면 주기 체크 로그와 알림 없이 조용히 종료합니다. 매수가능 USD는 같은 미국 거래일의 첫 `inquire_psamount` 조회 결과를 재사용하여 계좌 API 호출을 반복하지 않습니다.
3. **지능형 알림**:
