...
3. **핸들러 등록**: system lifecycle, 포트폴리오, 전략, 메모 등 기능별
   핸들러를 등록합니다. Trading runtime이 OFF이면 매매/조회 명령은
   `/system_on` 안내로 차단됩니다.
4. **폴링 시작**: `Application.start_polling()`을 호출하여 메시지를 수신합니다.
5. **초기화 메시지**: 봇이 시작되었고 trading runtime은 OFF임을 알리며
   `/system_on` 중심의 초기 안내 메시지를 전송합니다. `/system_on` 성공
   뒤에는 기존 trading 명령 안내와 `/system_off` 안내를 함께 전송합니다.
...
### `telegram_system.register_system_handlers`
`/system_on`, `/system_off`, `/system_status` 명령을 등록합니다.
OFF 상태에서는 `/portfolio`, `/strategy`, `/rebalance`, `/daily_report` 등
trading runtime이 필요한 명령을 실행하지 않고 `/system_on` 안내를 보냅니다.

### `initialize_telegram` 내부 `run_bot`
`initialize_telegram()` 안에서 정의되는 중첩 함수로, 실제 비동기 루프를 관리합니다. `asyncio.new_event_loop()`를 사용하여 스레드 안전성을 확보합니다.
