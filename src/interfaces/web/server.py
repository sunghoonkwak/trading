"""
Web interface adapter for the trading event viewer.
...
class WebDependencies:
    """Runtime collaborators supplied by ``main.py``."""

    runtime_is_running: Callable[[], bool]
    set_broadcast_callback: Callable[[Callable[..., None]], None]
...
def _env_flag(name: str, default: bool = False) -> bool:
    """Read an injected boolean feature flag."""
    return _require_dependencies().env_flag(name, default)
...
def _is_runtime_running() -> bool:
    return _require_dependencies().runtime_is_running()


def _get_portfolio_reader() -> Any:
    return _require_dependencies().portfolio_reader
...
async def get_memos():
    ...
    try:
        return _require_dependencies().load_memos()
    except Exception as e:
...
async def delete_memo(request: MemoDeleteRequest):
    ...
    try:
        messages = _require_dependencies().load_memos()
        ...
        if date_key in messages:
            ...
            if len(messages[date_key]) < original_len:
                ...
                _require_dependencies().save_memos(messages)
...
async def trigger_portfolio_report(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            _require_dependencies().run_portfolio_report,
            _get_portfolio_reader(),
        )
...
async def trigger_order_report(background_tasks: BackgroundTasks):
    ...
    try:
        # Run in background to not block response
        background_tasks.add_task(_require_dependencies().run_order_report)
