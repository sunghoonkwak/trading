...
1. **KIS 원천 조회 위임**: `broker.kis_portfolio`가 KIS API 데이터를
   표준 source 포맷으로 변환해 제공합니다.
2. **GSheet 원천 조회**: `data.gsheet`을 통해 수동/외부 자산을 읽습니다.
3. **Toss 원천 대체**: `broker.toss_portfolio`가 Toss API 데이터를
   표준 source 포맷으로 변환해 제공합니다. Toss 조회가 실패하면
   GSheet의 `토스_owner_01` 데이터를 fallback으로 유지하고
   `metadata.toss_error`를 남깁니다.
...
source 데이터로 대체합니다. 현재는 `broker.toss_portfolio`가 Toss API
데이터를 성공적으로 읽었을 때 `토스_owner_01` 계정에 사용합니다.
