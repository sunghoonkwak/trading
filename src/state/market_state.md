...
### `get_ticker_data` / `get_all_market_data`
모듈 레벨 alias로 특정 종목 또는 전체 시장 데이터를 안전하게 가져옵니다.
클래스 인스턴스를 직접 사용할 때는 `MarketStateManager.get_ticker()`와
`MarketStateManager.get_price()`를 호출할 수 있습니다.

### `get_market_manager`
싱글톤 `MarketStateManager` 인스턴스를 반환합니다. 명시 저장이나 종료 전
flush가 필요할 때는 이 인스턴스의 `save_to_disk()` 또는
`stop_persistence()`를 호출합니다.
