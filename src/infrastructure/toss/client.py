...
from infrastructure.toss.rate_limit import DEFAULT_RATE_LIMIT_MANAGER, TossRateLimitManager

_failure_notifier: Callable[[str], None] | None = None


def configure_failure_notifier(notifier: Callable[[str], None] | None) -> None:
    """Inject best-effort notification delivery from the composition root."""
...
def _send_failure_notification(
    *,
    ...
    try:
        sender = notify_func or _failure_notifier
        if sender is None:
            return
        sender(
