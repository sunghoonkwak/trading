...
# Compatibility patch: these WebSocket record transformations are required by
# the upstream callback path. Keeping them in this deliberate vendor patch
# avoids adding an application-owned module inside the vendor distribution.
_SENSITIVE_WS_COLUMNS = {"CUST_ID", "ACNT_NO", "ACNT_NO2", "ODER_NO", "OODER_NO", "ACNT_NAME"}


def _mask_record_for_log(record, columns):
    return [
        "********" if (columns[index] if index < len(columns) else f"EXTRA_{index}") in _SENSITIVE_WS_COLUMNS else value
...
def _normalize_record(record, columns):
    ...
    if len(record) == expected:
        return record, None
    if len(record) < expected:
        return record + [""] * (expected - len(record)), (
            f"padded {expected - len(record)} missing field(s): {columns[len(record):]}"
        )
    return record[:expected], f"truncated {len(record) - expected} extra field(s)"


def _should_log_normalization(note, expected_truncation):
    return note is not None and not (expected_truncation and note.startswith("truncated "))


def _should_send_schema_drift_alert(sent_at, tr_id, now, interval_seconds=3600.0):
...
def _build_schema_drift_alert(tr_id, note, field_count, column_count):
    return (
...
def _split_records(raw_values, count, real_size):
    if count == 1:
        return [raw_values]
    return [
        raw_values[index:index + real_size]
        for index in range(0, len(raw_values), real_size)
        if len(raw_values[index:index + real_size]) == real_size
    ]

# The KIS sample remains independently importable. Application-owned logging,
# credentials, state, and notifications are optional collaborators registered
# by an infrastructure adapter at composition time.
_credential_provider: Callable[[], tuple[str | None, str | None, str | None]] | None = None
_api_request_logger: Callable[..., None] | None = None
_api_response_logger: Callable[[object], None] | None = None
_websocket_send_logger: Callable[[dict], None] | None = None
_critical_error_handler: Callable[[str], None] | None = None
_websocket_event_handler: Callable[..., None] | None = None


def configure_hooks(
    *,
    credential_provider: Callable[[], tuple[str | None, str | None, str | None]] | None = None,
    api_request_logger: Callable[..., None] | None = None,
    api_response_logger: Callable[[object], None] | None = None,
    websocket_send_logger: Callable[[dict], None] | None = None,
    critical_error_handler: Callable[[str], None] | None = None,
    websocket_event_handler: Callable[..., None] | None = None,
) -> None:
    """Register optional host callbacks without importing host packages."""
    global _credential_provider, _api_request_logger, _api_response_logger
    global _websocket_send_logger, _critical_error_handler, _websocket_event_handler
    _credential_provider = credential_provider
    _api_request_logger = api_request_logger
    _api_response_logger = api_response_logger
    _websocket_send_logger = websocket_send_logger
    _critical_error_handler = critical_error_handler
    _websocket_event_handler = websocket_event_handler


def _emit_websocket_event(event: str, **payload) -> None:
    if _websocket_event_handler is None:
        return
    try:
        _websocket_event_handler(event, **payload)
    except Exception as exc:
        logging.debug("KIS WebSocket event hook failed: %s", exc)
...
def _get_config():
    ...
    if _credential_provider is None:
        app_key = app_secret = app_hts_id = None
    else:
        app_key, app_secret, app_hts_id = _credential_provider()
...
def _handle_critical_error(msg):
    logging.error(msg)
    if _critical_error_handler is not None:
        _critical_error_handler(msg)
...
class APIResp:
    ...
    def printAll(self):
        if _api_response_logger is not None:
            _api_response_logger(self)
...
def _url_fetch(
    ...
    if _DEBUG:
        if _api_request_logger is not None:
            _api_request_logger(url=url, tr_id=tr_id, headers=headers, body=params)
...
def data_fetch(tr_id, tr_type, params, appendHeaders=None) -> dict:
    ...
    if _DEBUG:
        if _api_request_logger is not None:
            _api_request_logger(tr_id=tr_id, headers=headers)
...
class KISWebSocket:
    ...
    async def __subscriber(self, ws: websockets.ClientConnection):
        async for raw in ws:
            ...
            if raw[0] in ["0", "1"]:
                ...
                for record in _split_records(raw_values, count, real_size):
                    ...
                    raw_record_for_log = _mask_record_for_log(record, dm["columns"])
                    record, normalization_note = _normalize_record(record, dm["columns"])
                    expected_truncation = bool(fix and fix.get("expected_truncation"))
                    if _should_log_normalization(normalization_note, expected_truncation):
                        logging.warning(
                        ...
                        if _should_send_schema_drift_alert(
                            _schema_drift_alert_sent_at,
                            tr_id,
                            time.time(),
                        ):
                            self._send_telegram_notification(
                                _build_schema_drift_alert(
                                    tr_id,
                                    normalization_note,
    ...
    def _add_alert(self, message: str, level: str = "INFO"):
        """Publish a vendor-neutral WebSocket alert event."""
        _emit_websocket_event("alert", message=message, level=level)

    def _send_telegram_notification(self, message: str):
        """Publish a preformatted notification event for the registered host."""
        _emit_websocket_event("notification", message=message)

    def _update_ws_status(self, status: str):
        """Publish a vendor-neutral connection-status event."""
        _emit_websocket_event("status", status=status)
    ...
    async def __runner(self):
        ...
        while not self._stop_requested:
            try:
                ...
                async with websockets.connect(url, ping_interval=60, ping_timeout=120) as ws:
                    ...
                    if self.retry_count >= 3:
                        _emit_websocket_event(
                            "reconnection_success", failed_attempts=self.retry_count
                        )
                    ...
                    for _name, obj in open_map.items():
                ...
                if attempt_number >= 3:
                    _emit_websocket_event(
                        "reconnection_failure", attempt_number=attempt_number, error=e
                    )
            except Exception as e:
                self._update_ws_status("reconnecting")
            ...
            try:
                ...
                _emit_websocket_event("approval_refresh_failure", error=e)
    ...
    async def send(
        ...
        if _websocket_send_logger is not None:
            _websocket_send_logger(msg)
