...
### initialize_telegram
텔레그램 봇을 초기화하고 백그라운드 스레드에서 폴링을 시작합니다. `asyncio.run()`을 사용하여 봇 스레드 내에서 독립적이고 안전한 이벤트 루프 라이프사이클을 관리합니다. `initialize` -> `start` -> `polling` 순서의 표준 비동기 시퀀스를 따르며, 전역 에러 핸들러를 등록하여 예기치 못한 운영 오류를 시스템 UI 알림 영역에 실시간으로 보고합니다.
