...
def test_retired_utils_logger_module_has_no_active_python_consumers():
    assert not (SRC_DIR / "utils/logger.py").exists()
    assert not (SRC_DIR / "utils/logger.md").exists()
    ...
    for root in roots:
        for path in root.rglob("*.py"):
            if path == Path(__file__):
                continue
            if "utils.logger" in path.read_text(encoding="utf-8"):
                consumers.append(path.relative_to(SRC_DIR.parent).as_posix())

    assert consumers == []


def test_retired_core_thread_comm_module_has_no_active_python_consumers():
    assert not (SRC_DIR / "core/thread_comm.py").exists()
    assert not (SRC_DIR / "core/thread_comm.md").exists()
    ...
    for root in roots:
        for path in root.rglob("*.py"):
            if path == Path(__file__):
                continue
            if "core.thread_comm" in path.read_text(encoding="utf-8"):
                consumers.append(path.relative_to(SRC_DIR.parent).as_posix())

    assert consumers == []


def test_retired_core_lock_manager_module_has_no_active_python_consumers():
    assert not (SRC_DIR / "core/lock_manager.py").exists()
    assert not (SRC_DIR / "core/lock_manager.md").exists()
    ...
    for root in roots:
        for path in root.rglob("*.py"):
            if path == Path(__file__):
                continue
            if "core.lock_manager" in path.read_text(encoding="utf-8"):
                consumers.append(path.relative_to(SRC_DIR.parent).as_posix())

    assert consumers == []


def test_retired_trading_configuration_modules_have_no_active_consumers():
    for relative_path in (
        "core/trading_config.py",
        "core/trading_config.md",
        "infrastructure/stock_configuration.py",
        "infrastructure/stock_configuration.md",
    ):
        assert not (SRC_DIR / relative_path).exists()
    ...
    for root in roots:
        for path in root.rglob("*.py"):
            ...
            if "core.trading_config" in source or "infrastructure.stock_configuration" in source:
                consumers.append(path.relative_to(SRC_DIR.parent).as_posix())
...
def test_retired_core_display_module_has_no_active_python_consumers():
    assert not (SRC_DIR / "core/display.py").exists()
    assert not (SRC_DIR / "core/display.md").exists()
...
def test_retired_system_state_module_has_no_active_python_consumers():
    assert not (SRC_DIR / "state/system_state.py").exists()
    assert not (SRC_DIR / "state/system_state.md").exists()

    consumers = []
    for root in [SRC_DIR, SRC_DIR.parent / "tests", SRC_DIR.parent / "scripts"]:
        for path in root.rglob("*.py"):
            if path != Path(__file__) and "state.system_state" in path.read_text(encoding="utf-8"):
                consumers.append(path.relative_to(SRC_DIR.parent).as_posix())
