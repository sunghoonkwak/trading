...
#### output
- `tuple[DataFrame, str]`: (결과 데이터프레임, 감지된 시장 코드 "KR" 또는 "US").
...
### sync_open_orders
둘러보기(Manage Orders) 진입 시 또는 실시간 이벤트 수신 시 호출되어 서버의 미체결 내역을 메인 UI의 하단 목록에 동기화합니다. 기존 목록을 초기화한 후 최신 데이터만 반영하며, Event Viewer의 혼잡을 줄이기 위해 동기화 로그는 전송하지 않습니다.
#### input
- `None`.
#### output
- `None`.
