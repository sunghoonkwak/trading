...
3. 당일 사용가능한 자금(seed/duration)을 아래와 같은 비율로 매수계산합니다.
    - initial상태(보유주식이 없는상태): 금일 현재가 * 110%의 price로 당일 사용가능한 자금 전체를 LOC 매수합니다.
    - 보유상태:
        a. 1/2: 내 평단가 * 115%의 price로 LOC 매수  ## 115%보다 낮으면 종가에 매수되어 무조건 매수
        b. 1/2: 내 평단가의 price로 LOC 매수         ## 내 평단가보다 낮을경우 매수하여 평단가를 낮추기 위해
4. 내 평단가 * 110%(평단가 + 10%)의 price로 보유한 주식 전체를 LOC 매도합니다.
